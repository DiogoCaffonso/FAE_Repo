...
void analise() {
    ...
    for (const auto& massa : e_massas_invariantes) if (massa >= 80) hEletron->Fill(massa);
    for (const auto& massa : m_massas_invariantes) if (massa >= 80) hMuon->Fill(massa);
    for (const auto& massa : t_massas_invariantes) if (massa >= 80) hTau->Fill(massa);
